...
export class CollegeRingSeriesNftsPoller {
...
export class CollegeRingSeriesNftsPollerFactory {
	static async runPoller(eventsDirectory: string, chainId: number, db: admin.firestore.Firestore, apiKey: string): Promise<CollegeRingSeriesNftsPoller> {
		const pollerInstance = new CollegeRingSeriesNftsPoller(eventsDirectory, chainId, db);
